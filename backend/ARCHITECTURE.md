...
- **Artifacts**: `backend/data/artifacts/{enrollment_id}/...` (local disk today) moves to S3-compatible object storage (Cloudflare R2 recommended — see cost table) — required as soon as there's more than one API/worker replica, since local disk isn't shared across them.
