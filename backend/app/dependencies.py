...
from app.models_rbac import AdminRolePermission
...
def require_admin_or_superadmin(token: dict = Depends(get_current_user)) -> dict:
    if token.get("role") not in ("SUPER_ADMIN", "ADMIN"):
