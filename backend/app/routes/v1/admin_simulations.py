...
Gated by granular simulations.* permissions (see
...
AdminRole. Available in the Admin portal, not just SuperAdmin's.
"""
...
async def preview_full_simulation(sim_id: str, db: AsyncSession = Depends(get_db), _=Depends(require_permission("simulations.view"))):
    ...
    but works for DRAFT simulations too — gated by simulations.view instead
...
async def unenroll_all_students(
    ...
    """Bulk-remove every student's enrollment in this simulation — the way
    to clear the block delete_simulation raises above ("existing
    enrollments") without deleting students one at a time. Mirrors
    admin.py's remove_enrollment (revoke the journey badge, then drop the
    Enrollment row) but for every enrollment in this sim at once. Same
    caveat applies here as there: this wipes real student progress and
    can't be undone, so the frontend gates it behind an explicit confirm
    step naming the number of students affected."""
    await _get_sim_or_404(sim_id, db)
    result = await db.execute(select(Enrollment).where(Enrollment.simulation_id == sim_id))
    ...
    await log_action(
        ...
        action="simulation.unenroll_all", target_type="simulation", target_id=sim_id,
        meta={"unenrolled_count": len(enrollments), "user_ids": user_ids},
    )
    await db.execute(delete(Enrollment).where(Enrollment.simulation_id == sim_id))
    await db.commit()
...
async def update_task(sim_id: str, task_id: str, body: SimulationTaskUpdate, db: AsyncSession = Depends(get_db), _=Depends(require_permission("simulations.edit"))):
    task = await _get_task_or_404(sim_id, task_id, db)
    patch = body.model_dump(exclude_unset=True)
...
async def duplicate_task(sim_id: str, task_id: str, db: AsyncSession = Depends(get_db), _=Depends(require_permission("simulations.edit"))):
...
async def delete_task(sim_id: str, task_id: str, db: AsyncSession = Depends(get_db), _=Depends(require_permission("simulations.edit"))):
    task = await _get_task_or_404(sim_id, task_id, db)
