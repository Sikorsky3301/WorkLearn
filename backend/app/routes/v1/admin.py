...
from app.routes.v1.enrollments import JOURNEY_BADGE_KEY
...
async def admin_users(
    ...
    # Admin-tier accounts (ADMIN/SUPER_ADMIN) never show up here — this list
    # backs the student-facing "Users"/"Direct Users"/"Mentors" pages in both
    # the Admin and SuperAdmin portals, and an ADMIN with users.view has no
    # business seeing other admins' usernames. Admin accounts are managed
    # exclusively through admin_management.py's list_admins, gated on its own
    # admins.view permission.
    q = select(User).where(User.role.notin_([Role.ADMIN, Role.SUPER_ADMIN]))
    if role:
        try:
...
async def unlock_feature(user_id: str, body: dict, db: AsyncSession = Depends(get_db), token: dict = Depends(require_permission("users.unlock_feature"))):
    ...
    if not existing.scalar_one_or_none():
        ...
        await log_action(
            ...
            action="user.unlock_feature", target_type="user", target_id=user_id, meta={"feature": feature},
        )
        await db.commit()
    return {"ok": True}

@router.get("/users/{user_id}/enrollments")
async def user_enrollments(user_id: str, db: AsyncSession = Depends(get_db), token: dict = Depends(require_permission("users.view"))):
...
async def suspend_user(user_id: str, db: AsyncSession = Depends(get_db), token: dict = Depends(require_permission("users.suspend"))):
    """Real suspend/activate — previously only hard delete existed for taking
    a user out of action."""
    result = await db.execute(select(User).where(User.id == user_id))
    user = result.scalar_one_or_none()
    if not user:
        raise HTTPException(404, "User not found")
    ...
    await log_action(
        db, actor_id=actor_id, actor_role=actor_role, actor_name=actor_name,
        action="user.suspend", target_type="user", target_id=user_id,
    )
    await db.commit()
    return {"ok": True, "is_active": user.is_active, "suspended_at": user.suspended_at}

@router.post("/users/{user_id}/activate")
async def activate_user(user_id: str, db: AsyncSession = Depends(get_db), token: dict = Depends(require_permission("users.suspend"))):
    result = await db.execute(select(User).where(User.id == user_id))
    user = result.scalar_one_or_none()
    if not user:
        raise HTTPException(404, "User not found")
    ...
    await log_action(
        db, actor_id=actor_id, actor_role=actor_role, actor_name=actor_name,
        action="user.activate", target_type="user", target_id=user_id,
    )
    await db.commit()
    return {"ok": True, "is_active": user.is_active, "suspended_at": user.suspended_at}

@router.delete("/enrollments/{enrollment_id}")
async def remove_enrollment(enrollment_id: str, db: AsyncSession = Depends(get_db), token: dict = Depends(require_permission("users.edit"))):
    ...
    await log_action(
        db, actor_id=actor_id, actor_role=actor_role, actor_name=actor_name,
        action="enrollment.remove", target_type="enrollment", target_id=enrollment_id,
...
async def delete_user(user_id: str, db: AsyncSession = Depends(get_db), token: dict = Depends(require_permission("users.delete"))):
