...
export function useUpdateSimBuilderProject(projectId) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (body) => api.patch(`/api/admin/sim-builder/${projectId}`, body),
    onSuccess: () => {
...
export function useUpdateSimBuilderPage(projectId) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ pageId, ...body }) => api.patch(`/api/admin/sim-builder/${projectId}/pages/${pageId}`, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['sim-builder-project', projectId] }),
  })
}
