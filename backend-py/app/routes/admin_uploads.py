...
Superadmin-only, saved to a locally-served static directory and returned as
a URL the rest of the CMS (Simulation.logo_url, manager.photo_url) just
treats as an opaque string, same as any other image URL.
"""
import uuid
...
@router.post("/image")
async def upload_image(file: UploadFile = File(...), _=Depends(require_superadmin)):
