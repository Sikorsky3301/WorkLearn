...
and stageQuizzes.js are frontend JS, not importable Python:
  - backend-py/app/routes/enrollments.py (SIMULATIONS, SIM_MANAGERS,
    SIM_TASK_BRIEFS, SIM_ONBOARDING)
  - backend-py/app/config.py (SIM_TASK_XP_AWARDS, SIM_TASK_SKILL_AWARDS,
    SIM_TASK_WEEKS via SIM_TASK_NAMES's task-0 "Onboarding" entries)
  - backend-py/app/routes/sandbox.py (SIM_TASK_IO) — informs grader_key/
    dataset_key/language per code_sandbox task
  - backend-py/app/services/crm_sim_prompts.py (PERSONALITIES, EMAIL_GRADING_PROMPT)
...
Run with: python migrate_legacy_sims.py  (from backend-py/, so app/ resolves
...
FE_TASK3_STARTER = """// renderDirectory(container, fetchFn) is called by the grader with a real
...
FE_TASK4_STARTER = """import React from 'react';
...
FE_TASK5_STARTER = """import React, { useState } from 'react';
...
FE_TASK1_MODEL_SOLUTION_CODE = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Enigma</title>
...
FE_TASK2_MODEL_SOLUTION_CODE = """<!DOCTYPE html>
<html>
...
FE_TASK3_MODEL_SOLUTION_CODE = """function renderDirectory(container, fetchFn) {
...
FE_TASK4_MODEL_SOLUTION_CODE = """import React from 'react';
...
FE_TASK5_MODEL_SOLUTION_CODE = """import React, { useState, useEffect } from 'react';
...
# The 5-question cumulative assessment shown once after Task 5 (replacing the
# certificate view) — not a per-task quiz like da-job-sim/sales-crm-sim have.
# Positionally equivalent to Task 5's post_task_quiz (it's the last thing shown
# before the certificate), even though its content spans all 3 weeks — a
# deliberate approximation, not a literal transcription of "per-task" scope.
FE_FINAL_ASSESSMENT = {
    "questions": [
        {"question": "Why use <header>, <nav>, <main>, and <footer> instead of <div> elements with matching class names?",
         "options": ["They render faster in the browser",
                     ...
                     "They automatically apply default browser styling that looks better"], "correct": 1},
        {"question": "What is the correct, accessible way to implement a toggle-able mobile menu button?",
         "options": ["Just hide/show the menu with CSS — no JavaScript needed",
                     ...
                     "Reload the page with a different URL when the button is clicked"], "correct": 1},
        {"question": "A component fetches data from an API. Which of these is the most correct UI to build?",
         "options": ["Only render the data once it arrives — showing nothing until then is simplest",
                     ...
                     "Retry the request silently forever until it succeeds"], "correct": 1},
        {"question": "A React component needs to display \"loading\" and \"error\" states, but doesn't fetch any data itself. How should those states reach it?",
         "options": ["As props, passed down from whatever component owns the fetch",
                     "As local state initialized to false", "Via a global variable set from outside React",
                     "It should call fetch itself so it always has the freshest state"], "correct": 0},
        {"question": "You need a piece of React state to survive a full page reload. What's the correct pattern?",
         "options": ["Increase the component's re-render frequency so state updates propagate faster",
                     ...
                     "Nothing needed — React state persists across reloads automatically"], "correct": 2},
    ],
}
...
def frontend_dev_sim():
    sim = dict(
        ...
        manager={"name": "Maya Chen", "role": "Frontend Engineering Lead", "avatar": "MC", "photo_url": "/static/seed/maya-chen.jpg"},
        onboarding={
            "company": {
                "name": "Enigma", "industry": "B2B SaaS · Productivity Software",
                "size": "~85 employees", "location": "Remote-first · US/EU",
                "about": "Enigma builds a workspace platform teams use to plan, track, and ship their work in one "
                         "place. The Web Platform team owns everything the customer actually sees and clicks — "
                         "performance and polish are the product.",
            },
            "intro": (
                "Hey, welcome to the Web Platform team — excited to have you.\n\n"
                "Here's how I work: I'll hand you tickets exactly like I would to any frontend engineer on the "
                "team. Build the real thing, don't just make it look right — I'll be checking that it actually "
                "behaves correctly, not just that it renders.\n\n"
                "You'll start with the fundamentals — markup and layout — and build up to a real interactive React "
                "feature by the end. Let's get you set up."
            ),
            "learn": [
            ...
            "offer": {
                ...
                "body": (
                    "We're delighted to offer you a place on the Enigma Frontend Developer Job Simulation. You'll "
                    "work directly with your manager on five real tickets, going from a static landing page to a "
                    "fully interactive React application, building the exact skills a frontend developer needs on "
                    "the job. By accepting, you're committing to give each task a genuine attempt and to learn by "
    ...
    common = dict(submission_mode="code", grading_strategy="registered_grader")

    tasks = [
        dict(task_index=1, title="Task 1 — Landing Hero Section", type="code_sandbox", week=1,
             objective="New landing page — first impression matters",
             briefing="Marketing's been waiting on this for weeks — we need a hero section for the new landing "
                      "page before the launch newsletter goes out. Semantic HTML and solid CSS layout, nothing "
                      "fancy yet. No JavaScript needed for this one — I just want a page that reads cleanly and "
                      "holds together on any screen size.",
             what_to_do=["Build the page structure with real semantic tags: a <header> containing a <nav>, a "
                         "<main> with a hero <section>, and a <footer>. No generic <div> soup.",
                         "Your <nav> needs at least one visible, readable link — not an empty tag or an icon with "
                         "no text.",
                         "Inside <main>, your hero <section> needs exactly one <h1> — that's your headline.",
                         "Lay out the hero with CSS Flexbox or Grid in an embedded <style> block (display: flex "
                         "or display: grid somewhere in your CSS)."],
             what_to_submit=["A single submission.html file — HTML, and CSS in an embedded <style> block. No "
                              "separate files, no build step."],
             hints=["The grader checks for real tags, not classes — it looks for <header>, <main>, <footer>, and "
                    'a <nav> with at least one <a>. Div-with-a-className-of-"header" won\'t pass.',
                    "Exactly one <h1> inside <main> — if you have zero, or more than one, that check fails. "
                    "Everything else can be <h2> or lower."],
             success_criteria=["Semantic header/nav/main/footer structure", "Nav has a visible link",
                                "Exactly one <h1>", "Flexbox or Grid layout used"],
             model_solution={
                 "steps": [
                     dict(title="Structure with real landmarks",
                          detail="header > nav, main > section#hero, footer — not divs with matching class names. "
                                 "Screen readers and the grader both rely on the actual tag.",
                          example="<header>\n  <nav>\n    <a href=\"#hero\">Home</a>\n    <a href=\"#features\">Features</a>\n"
                                  "  </nav>\n</header>\n<main>\n  <section id=\"hero\">\n    <h1>Build better, together.</h1>\n"
                                  "    <p>Enigma is the workspace your team actually wants to use.</p>\n  </section>\n</main>\n"
                                  "<footer>&copy; 2026 Enigma</footer>"),
                     dict(title="Lay out the hero with Flexbox or Grid",
                          detail="Grid is a clean fit for centering a hero block; Flexbox works just as well for "
                                 "the header row. Either satisfies the check.",
                          example="header {\n  display: flex;\n  justify-content: space-between;\n  align-items: center;\n"
                                  "  padding: 1rem;\n}\n#hero {\n  display: grid;\n  place-items: center;\n"
                                  "  min-height: 60vh;\n  text-align: center;\n}"),
                     dict(title="Give the nav real, visible link text",
                          detail="Icon-only or empty links fail accessibility in the real world and fail the "
                                 "automated check here for the same reason.", example=None),
                 ],
                 "key_principle": "Semantic HTML is not a style preference — it's what makes a page navigable by "
                                   "screen readers, crawlable by search engines, and testable by tools like this one.",
                 "great_looks_like": "A page that reads correctly even with CSS turned off — the HTML structure "
                                      "alone tells you what everything is.",
                 "example_solution": FE_TASK1_MODEL_SOLUTION_CODE,
             },
             config={**common, "language": "html", "grader_key": "frontend_dev_sim.task1", "starter_code": FE_TASK1_STARTER,
                     "input_filename": "submission.html", "output_filename": "output.json"},
             xp_award=50, skill_awards={"html_css": 15, "accessibility": 10}),
        dict(task_index=2, title="Task 2 — Interactive Navigation", type="code_sandbox", week=1,
             objective="Nav needs to actually work on mobile",
             briefing="QA flagged that our nav is just static markup right now — no mobile menu, no way to tell "
                      "which page you're on. Can you wire it up? Standard pattern: a toggle button that "
                      "opens/closes the menu and stays accessible, plus highlighting whichever link is active. "
                      "This is vanilla JS — no framework needed yet.",
             what_to_do=['Your toggle button must have id="nav-toggle", start with aria-expanded="false", and '
                         'reference aria-controls="nav-menu".',
                         'Your menu container must have id="nav-menu" and contain at least two links with '
                         'class="nav-link".',
                         'Clicking the toggle must flip aria-expanded between "true" and "false", and add/remove '
                         "a CSS class on #nav-menu (however you want to show/hide it).",
                         'Clicking a .nav-link must mark that link "active" (add the class active) and remove '
                         "active from every other link."],
             what_to_submit=["A single submission.html file — markup plus an inline <script> at the end of "
                              "<body>."],
             hints=["Attach your listeners after the elements exist in the DOM — put your <script> at the end of "
                    "<body>, or wrap your logic in a DOMContentLoaded listener.",
                    "The grader executes your real script in a real DOM and clicks the actual button — "
                    "console.log-only \"it should work\" code won't pass."],
             success_criteria=["Toggle flips aria-expanded", "Menu shows/hides", "Active link highlighting works"],
             model_solution={
                 "steps": [
                     dict(title="Wire the toggle button",
                          detail="Read the current aria-expanded value, flip it, and write it back — don't "
                                 "hardcode \"true\", or a second click won't undo the first.",
                          example="var toggle = document.getElementById('nav-toggle');\n"
                                  "var menu = document.getElementById('nav-menu');\n"
                                  "toggle.addEventListener('click', function () {\n"
                                  "  var expanded = toggle.getAttribute('aria-expanded') === 'true';\n"
                                  "  toggle.setAttribute('aria-expanded', String(!expanded));\n"
                                  "  menu.classList.toggle('open');\n});"),
                     dict(title="Highlight the active link",
                          detail="Clear \"active\" from every link first, then add it to only the one that was "
                                 "clicked — otherwise old highlights stick around.",
                          example="var links = menu.querySelectorAll('.nav-link');\n"
                                  "links.forEach(function (link) {\n"
                                  "  link.addEventListener('click', function () {\n"
                                  "    links.forEach(function (l) { l.classList.remove('active'); });\n"
                                  "    link.classList.add('active');\n  });\n});"),
                 ],
                 "key_principle": "aria-expanded isn't decoration — assistive tech reads it to announce whether "
                                   "the menu is open. A visually-correct menu with the wrong aria-expanded value is "
                                   "still broken for screen-reader users.",
                 "great_looks_like": "Click the toggle in the live preview and watch it actually work — menu "
                                      "opens, aria-expanded flips, clicking a link highlights it and only it.",
                 "example_solution": FE_TASK2_MODEL_SOLUTION_CODE,
             },
             config={**common, "language": "html", "grader_key": "frontend_dev_sim.task2", "starter_code": FE_TASK2_STARTER,
                     "input_filename": "submission.html", "output_filename": "output.json"},
             xp_award=80, skill_awards={"javascript": 15, "accessibility": 10}),
        dict(task_index=3, title="Task 3 — Fetch & Render Data", type="code_sandbox", week=2,
             objective="Team directory page — needs to handle real network conditions",
             briefing="Next up: the team directory. It pulls from our internal API, which means it can be slow or "
                      "it can fail — and right now the page just shows a blank screen while it's loading and "
                      "silently breaks if the request errors. Fix that. Users should always see something "
                      "meaningful: loading, the data, or a clear error.",
             what_to_do=["Export a function renderDirectory(container, fetchFn) from submission.js — container "
                         "is a real DOM element, fetchFn is an injectable fetch-like function so it can be tested "
                         "without a real network.",
                         "The moment renderDirectory is called — before awaiting anything — show a loading "
                         'element with data-testid="loading".',
                         'On success, replace it with a <ul data-testid="team-list"> containing one <li> per '
                         "team member, showing their name.",
                         "On failure (the fetch call rejects), replace the loading state with an element with "
                         'data-testid="error" — never leave the loading state stuck or throw an uncaught error.'],
             what_to_submit=["A single submission.js file exporting renderDirectory via module.exports."],
             hints=["fetchFn behaves like the real fetch: fetchFn(url) returns a Promise that resolves to "
                    "something with a .json() method (also async).",
                    "Wrap the fetch + render in a try/catch (or a .catch on the promise chain) — an unhandled "
                    "rejection means the error state never shows."],
             success_criteria=["Loading state shown synchronously", "Success renders team list",
                                "Failure shows error state"],
             model_solution={
                 "steps": [
                     dict(title="Show loading synchronously",
                          detail="Set the loading markup before you call fetchFn, not inside a .then() — the "
                                 "grader checks for it immediately after calling your function, before the fetch resolves.",
                          example="function renderDirectory(container, fetchFn) {\n"
                                  "  container.innerHTML = '<p data-testid=\"loading\">Loading...</p>';\n"
                                  "  return fetchFn('/api/team')\n    .then(function (res) { return res.json(); })\n"
                                  "    .then(function (data) {\n"
                                  "      var items = data.map(function (m) { return '<li>' + m.name + '</li>'; }).join('');\n"
                                  ...
                                  "      container.innerHTML = '<p data-testid=\"error\">Failed to load team directory.</p>';\n"
                                  "    });\n}"),
                     dict(title="Return the promise",
                          detail="Returning the chain lets the caller (and the grader) await completion instead "
                                 "of racing the render.", example=None),
                 ],
                 "key_principle": "Every async UI has three states: loading, success, and error. Shipping only the "
                                   "success state is the single most common frontend bug in real production code.",
                 "great_looks_like": "The function never leaves the user staring at a blank screen, no matter "
                                      "what the network does.",
                 "example_solution": FE_TASK3_MODEL_SOLUTION_CODE,
             },
             config={**common, "language": "javascript", "grader_key": "frontend_dev_sim.task3", "starter_code": FE_TASK3_STARTER,
                     "input_filename": "submission.js", "output_filename": "output.json"},
             xp_award=90, skill_awards={"javascript": 15, "async_data": 15}),
        dict(task_index=4, title="Task 4 — React Component", type="code_sandbox", week=2,
             objective="Port the directory to React — we're migrating the whole app",
             briefing="Good news: the vanilla-JS directory page works, and now we're porting it into the new "
                      "React shell. Convert what you built into a proper component. Same three states as before — "
                      "loading, error, data — but now driven by props instead of manual DOM writes.",
             what_to_do=["Default-export a functional component EmployeeList({ data, loading, error }) from "
                         "submission.jsx.",
                         'When loading is true, render an element with data-testid="loading".',
                         "When error is set (a non-empty string), render an element with "
                         'data-testid="error" whose text is the error message.',
                         'Otherwise, render a <ul data-testid="employee-list"> with one entry per item in data, '
                         "showing each person's name."],
             what_to_submit=["A single submission.jsx file, default-exporting EmployeeList."],
             hints=["Check loading and error before falling through to the list — order matters.",
                    "The grader renders your component directly with React Testing Library and checks for these "
                    "three states independently."],
             success_criteria=["Loading state via prop", "Error state via prop", "Data list rendered"],
             model_solution={
                 "steps": [
                     dict(title="Branch on loading and error first",
                          detail="A component with three possible outputs should make each one an explicit early "
                                 "return — much easier to reason about than nested ternaries.",
                          example="export default function EmployeeList({ data, loading, error }) {\n"
                                  "  if (loading) {\n    return <p data-testid=\"loading\">Loading...</p>;\n  }\n"
                                  "  if (error) {\n    return <p data-testid=\"error\">{error}</p>;\n  }\n"
                                  "  return (\n    <ul data-testid=\"employee-list\">\n"
                                  "      {(data || []).map(function (emp) {\n"
                                  "        return <li key={emp.id}>{emp.name} — {emp.role}</li>;\n      })}\n"
                                  "    </ul>\n  );\n}"),
                     dict(title="Props, not state",
                          detail="This component doesn't fetch anything itself — the loading/error/data values are "
                                 "handed to it. That's the whole point of the React conversion.", example=None),
                 ],
                 "key_principle": "A component that receives loading/error/data as props instead of managing its "
                                   "own fetch is easier to test, easier to reuse, and easier to reason about.",
                 "great_looks_like": "The exact same three states as Task 3, now expressed declaratively instead "
                                      "of with manual innerHTML writes.",
                 "example_solution": FE_TASK4_MODEL_SOLUTION_CODE,
             },
             config={**common, "language": "jsx", "grader_key": "frontend_dev_sim.task4", "starter_code": FE_TASK4_STARTER,
                     "input_filename": "submission.jsx", "output_filename": "output.json"},
             xp_award=100, skill_awards={"react": 20, "component_design": 15}),
        dict(task_index=5, title="Task 5 — Task Manager App", type="code_sandbox", week=3,
             objective="The capstone — a real stateful feature, start to finish",
             briefing="Last one, and it's the real deal: a task manager. Add tasks, mark them complete, delete "
                      "them — and it needs to survive a page reload, so whatever you build has to actually "
                      "persist. This is the closest thing to a real feature ticket you'll get in this simulation. "
                      "Take your time and build it properly.",
             what_to_do=["Default-export a functional component TaskManager from submission.jsx — no props "
                         "needed, it's self-contained.",
                         'Render an input[data-testid="task-input"] and a button[data-testid="add-task"] that '
                         "adds a new task from the input's current value.",
                         'Render a ul[data-testid="task-list"] of li[data-testid="task-item"] — one per task, '
                         "showing its text.",
                         'Each task item needs a button[data-testid="complete-task"] that toggles that task\'s '
                         'completed state, and a button[data-testid="delete-task"] that removes it.',
                         'Persist the tasks array to localStorage under the key "tasks" (as JSON: an array of '
                         "{ id, text, completed }) every time it changes, and rehydrate from localStorage on "
                         "first mount."],
             what_to_submit=["A single submission.jsx file, default-exporting TaskManager."],
             hints=["Initialize your useState from localStorage directly (a lazy initializer function), and use a "
                    "useEffect that writes to localStorage whenever the tasks array changes.",
                    "Give each task a stable, unique id when you create it."],
             success_criteria=["Add/complete/delete tasks", "Persists across reload"],
             model_solution={
                 "steps": [
                     dict(title="Load from localStorage on mount, save on every change",
                          detail="A lazy useState initializer runs once, reading whatever was persisted last time. "
                                 "The useEffect keyed on [tasks] keeps localStorage in sync going forward.",
                          example="function loadTasks() {\n  try {\n"
                                  "    return JSON.parse(window.localStorage.getItem('tasks') || '[]');\n"
                                  "  } catch (e) {\n    return [];\n  }\n}\n\n"
                                  "const [tasks, setTasks] = useState(loadTasks);\n\n"
                                  "useEffect(function () {\n  window.localStorage.setItem('tasks', JSON.stringify(tasks));\n}, [tasks]);"),
                     dict(title="Add, toggle, and delete — always through setTasks with a function updater",
                          detail="Using the functional form of setTasks (prev => ...) avoids stale-closure bugs "
                                 "when multiple updates happen in quick succession.",
                          example="function addTask() {\n  if (!text.trim()) return;\n"
                                  "  setTasks(function (prev) {\n"
                 ...
                 "key_principle": "State that needs to survive a reload has to be written somewhere outside of "
                                   "React's memory — localStorage is the simplest option, but the pattern (load "
                                   "once, save on every change) is the same one you'd use for any persistence layer.",
                 "great_looks_like": "Add a few tasks, reload the live preview tab (or come back later), and "
                                      "they're still there — that round-trip is the actual test of \"done.\"",
                 "example_solution": FE_TASK5_MODEL_SOLUTION_CODE,
             },
             config={**common, "language": "jsx", "grader_key": "frontend_dev_sim.task5", "starter_code": FE_TASK5_STARTER,
                     "input_filename": "submission.jsx", "output_filename": "output.json",
                     "post_task_quiz": FE_FINAL_ASSESSMENT},
             xp_award=120, skill_awards={"react": 15, "state_management": 25}),
    ]
    return sim, tasks
