...
export default function Dashboard() {
  ...
  const firstName    = user?.name?.split(' ')[0] || 'Rishi'
    ...
    <div className="max-w-container mx-auto px-6 py-8">
      ...
      <div className="mb-7">
        <div className="flex items-center gap-3 mb-1">
          <h1 className="text-3xl font-bold text-on-surface">Welcome, {firstName}!</h1>
          {isUniStudent && (
            <span className="flex items-center gap-1.5 bg-surface-low border border-border text-on-surface-variant text-xs font-semibold px-3 py-1 rounded-full">
        ...
        <p className="text-on-surface-variant text-sm">
          {isUniStudent
            ? 'Your mentor-assigned tasks and learning progress.'
            : 'Your AI-managed career roadmap for today. Stay on track to reach your Junior PM goals.'}
        </p>
      </div>

      {/* ── University student: Assigned Tasks ── */}
      {isUniStudent && user.assignedTasks?.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center justify-between mb-3">
            <span className="section-label">Assigned by your Mentor</span>
            <span className="text-xs text-on-surface-variant">{user.assignedTasks.filter(t => t.status === 'completed').length} of {user.assignedTasks.length} completed</span>
          </div>
            ...
            {user.assignedTasks.map(task => {
              ...
              return (
                <div
                  ...
                  onClick={() => task.resourceId === 'da-job-sim' ? navigate('/simulations/da-job-sim') : navigate(`/courses/${task.resourceId}`)}
                >
                  <div className={`h-1 w-full ${isCompleted ? 'bg-green-500' : 'bg-primary'}`} />
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-lg">{task.type === 'simulation' ? '🎮' : '📖'}</span>
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded ${isCompleted ? 'bg-green-100 text-green-700' : isInProgress ? 'bg-primary/10 text-primary' : 'bg-surface-high text-on-surface-variant'}`}>
                            {isCompleted ? 'Completed' : isInProgress ? 'In progress' : 'Pending'}
                          </span>
                        </div>
                        <h3 className="text-sm font-semibold text-on-surface group-hover:text-primary transition-colors leading-snug">{task.title}</h3>
                        {task.dueDate && (
                          <p className="text-xs text-on-surface-variant mt-0.5">Due {task.dueDate}</p>
                        )}
                      </div>
                      <button
                        className="btn-primary text-xs px-3 py-1.5 shrink-0 self-start"
                        onClick={e => { e.stopPropagation(); task.resourceId === 'da-job-sim' ? navigate('/simulations/da-job-sim') : navigate(`/courses/${task.resourceId}`) }}
                      >
          ...
          {/* Feature access status for university students */}
          <div className="mt-3 flex items-center gap-3 flex-wrap">
            <span className="text-xs text-on-surface-variant font-medium">Feature access:</span>
            {[
              ...
              { key: 'all_courses',    label: 'Full Catalog',   icon: '📚' },
            ].map(f => {
              const on = hasFeature(f.key)
              return (
                <span
                  key={f.key}
                  className={`flex items-center gap-1 text-xs px-2.5 py-1 rounded-full border font-medium ${on ? 'bg-green-50 border-green-200 text-green-700' : 'bg-surface-high border-border text-on-surface-variant'}`}
                >
                  {on ? '✓' : '🔒'} {f.icon} {f.label}
                </span>
              )
